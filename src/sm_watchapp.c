...
static char *status_layer_names[] = { "Battery Info", "" };
...
static TextLayer *text_mail_layer, *text_phone_layer, *text_battery_layer, *text_pebble_battery_layer;
...
static char battery_pct_str[5], batteryPbl_pct_str[5];
static char weather_wind_str[WIND_STRING_LENGTH], weather_humidity_str[HUMIDITY_STRING_LENGTH];
static char mail_count_str[BADGE_COUNT_STRING_LENGTH], phone_count_str[BADGE_COUNT_STRING_LENGTH];
...
static void rcv(DictionaryIterator *received, void *context) {
	...
	if (t != NULL) {
		...
		snprintf(batteryPbl_pct_str, sizeof(batteryPbl_pct_str), "%d", batteryPblPercent);
		text_layer_set_text(text_pebble_battery_layer, batteryPbl_pct_str);
	}
...
static void toggle_themes()
{
	...
	text_layer_set_text_color(text_pebble_battery_layer, textColor);
	text_layer_set_text_color(text_weather_humidity_layer, textColor);
...
static void up_double_click_handler (ClickRecognizerRef recognizer, void *context) {
	if (active_layer == MUSIC_LAYER) {
		set_info_text_with_timer("Volume up", date_switchback_short);
		sendCommand(SM_VOLUME_UP_KEY);
	} else if (active_status_layer == CALENDAR_STATUS_LAYER) {
...
/*	} else if (active_layer == WEATHER_LAYER) {
		set_info_text_with_timer("Garage press", date_switchback_long);
	} else if (active_layer == WEATHER_LAYER2) {
		set_info_text_with_timer("Activator Up", date_switchback_long);
...
/*static void up_long_release_handler(ClickRecognizerRef recognizer, void *context) {
	if (active_layer == WEATHER_LAYER) {
		sendCommandInt(SM_SCREEN_ENTER_KEY, URL_APP);
		psleep(1000);
		sendCommandInt(SM_REMINDERS_KEY, 0);
		psleep(1000);
		sendCommandInt(SM_URL1_KEY,0);
		psleep(2000);
		sendCommandInt(SM_URL1_KEY,1);
		psleep(1000);
		sendCommandInt(SM_SCREEN_ENTER_KEY, STATUS_SCREEN_APP);
		set_info_text_with_timer("Garage release", date_switchback_long);
	}
}
*/
...
/*	} else if (active_layer == WEATHER_LAYER2) {
		set_info_text_with_timer("Activator Select", date_switchback_long);
...
static void select_double_click_handler(ClickRecognizerRef recognizer, void *context) {
	if (active_layer == WEATHER_LAYER /*CALENDAR_LAYER && active_status_layer == CALENDAR_STATUS_LAYER */ ) {
...
/*	} else if (active_layer == WEATHER_LAYER) {
		set_info_text_with_timer("Preset test", date_switchback_long);
	} else if (active_layer == WEATHER_LAYER2) {
		set_info_text_with_timer("Activator Down", date_switchback_long);
...
static void down_long_release_handler(ClickRecognizerRef recognizer, void *context) {
	if (active_layer == WEATHER_LAYER) {
		...
		sendCommandInt(SM_REMINDERS_KEY, 0);
		psleep(1000);
		sendCommandInt(SM_URL1_KEY,3);
		psleep(2000);
		sendCommandInt(SM_URL1_KEY,2);
		psleep(1000);
		sendCommandInt(SM_SCREEN_ENTER_KEY, STATUS_SCREEN_APP);
		set_info_text_with_timer("Preset end", date_switchback_long);
	}
}
*/
static void click_config_provider(void *context) {
	...
	window_long_click_subscribe(BUTTON_ID_UP, 500, up_long_click_handler, NULL /* No handler on button release */);
	window_multi_click_subscribe(BUTTON_ID_UP, 2, 0, 0, true, up_double_click_handler);

	window_single_click_subscribe(BUTTON_ID_DOWN, down_click_handler);
	window_long_click_subscribe(BUTTON_ID_DOWN, 500, down_long_click_handler, NULL /* No handler on button release */);
...
void battery_layer_update_callback(Layer *me, GContext* ctx) {
	...
	graphics_fill_rect(ctx, GRect(32 - (int)((batteryPercent / 100.0)*32.0), 0, 32, 16), 0, GCornerNone);
}
...
void pebble_battery_layer_update_callback(Layer *me, GContext* ctx) {
	...
	graphics_fill_rect(ctx, GRect(32 - (int)((batteryPblPercent / 100.0)*32.0), 0, 32, 16), 0, GCornerNone);
}
...
static void init(void) {
	...
	text_pebble_battery_layer = text_layer_create(GRect(32, 23, 30, 48));
	text_layer_setup(status_layer[STATUS_LAYER], text_pebble_battery_layer, GTextAlignmentCenter, textColor, FONT_KEY_GOTHIC_18_BOLD, "");
	...
	text_battery_layer = text_layer_create(GRect(104, 23, 30, 48));
	...
	// Battery layers
	//---------------------------------------------------------------------------
	battery_layer = layer_create(GRect(103, 7, 32, 16));
	...
	pebble_battery_layer = layer_create(GRect(31, 7, 32, 16));
